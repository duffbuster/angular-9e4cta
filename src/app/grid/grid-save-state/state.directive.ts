...
export class IgxGridStateDirective implements AfterViewInit {
  ...
  public saveGridState() {
    ...
    const advancedFilteringState = {
      advancedFiltering: this.grid.advancedFilteringExpressionsTree
    };
  ...
  public restoreGridState() {
    ...
    if (this.filtering && this.gridState.filtering) {
      const filterTree = this.createExpressionsTreeFromObject(
        this.gridState.filtering
      );
    ...
    if (this.advancedFiltering && this.gridState.advancedFiltering) {
      const advFilterTree = this.createExpressionsTreeFromObject(
        this.gridState.advancedFiltering
      );
  ...
  private createExpressionsTreeFromObject(
    ...
    // console.log('top', exprTreeObject);
    if (!exprTreeObject || !exprTreeObject.filteringOperands) {
      return null;
    }

    const expressionsTree = new FilteringExpressionsTree(
      exprTreeObject.operator,
      exprTreeObject.fieldName
    );
    ...
    for (const item of exprTreeObject.filteringOperands) {
      ...
      if (item.filteringOperands) {
        const subTree = this.createExpressionsTreeFromObject(
          item as FilteringExpressionsTree
        );
        console.log('subTree', subTree);
        expressionsTree.filteringOperands.push(subTree);
        console.log('after push', expressionsTree.filteringOperands);
      } else {
        ...
        expr.searchVal =
          column.dataType === 'date'
            ? new Date(Date.parse(expr.searchVal))
            : expr.searchVal;
    ...
    console.log('bottom', expressionsTree);
